...
int main( int argc, char *argv[] ) {
	...
	r.read( data );
	data->SetSpacing(35,35,210);
	
	// cout << data->GetSpacing()[0] << endl;
	// cout << data->GetSpacing()[1] << endl;
	// cout << data->GetSpacing()[2] << endl;
	// cout << *data << endl;
	
	vtkImageData * tdata = vtkImageData :: New();
	tdata->SetDimensions(100,100,100);
	tdata->SetSpacing( 35, 35, 35 );
	// tdata->SetOrigin(-0*35,-0*35,-0*35);
	// tdata->SetExtent();

	vtkImageReslice * reslice = vtkImageReslice :: New();
	reslice->SetInput( data );
	reslice->SetBackgroundLevel( data->GetPointData()->GetScalars()->GetTuple1(0) );
	reslice->SetInformationInput( tdata );
