...
int main( int argc, char ** argv )
{
	...
	if ( argc < 9 ){
		cout << "USAGE: virus_segment_membrane_cylindrical input.mrc distance.mrc iradius oradius weight iterations variances output" << endl;
		return 1;
	}

	float innerRadius = atof( argv[3] );
	float outerRadius = atof( argv[4] );
	float weight = atof(argv[5]);
	int iterations = atoi( argv[6] );
	float varFactor = atof( argv[7] );
	float smooth = 1.0;
	...
	//	writermw.setFileName( argv[9] );
	//	writermw.write( saved );
	//}

	// get pressure volume from file
	mrc.setFileName( argv[2] );
	mrc.read(data);
	nbfVTKInterface :: vtkToBlitz( data, R );
	...
	writerm.setFileName( argv[8] );
	writerm.write( P );
	cout << "File " << argv[8] << " written.\nElapsed time = " << t.elapsedSeconds() << " seconds." << endl;
