...
str=sprintf('%s\nCXX=/usr/local/MPICH2/1.5/bin/mpic++ CXXFLAGS= -O3 -funroll-loops -fstrict-aliasing -fomit-frame-pointer -ffast-math', str);
str=sprintf('%s\nF77=gfortran FFLAGS=-fno-second-underscore -O3 -funroll-loops', str);
if length('/usr/bin/gfortran')
	str=sprintf('%s\nFC=/usr/bin/gfortran FCFLAGS=-fno-second-underscore -ffree-form -O3 -funroll-loops', str);
end
str=strrep(str,'_','\_');
h=text(0.5,0.5,str);
