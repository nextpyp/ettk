		...
		double *workspace )
{
  ...
  for( i = 0 ; i < bw - m - 1; i++ )
    {
      vec_mul_so3( L2_aN_so3(m+i, m1, m2), prevprev, tmp0, n );
      vec_mul_so3( L2_bN_so3(m+i, m1, m2), prev, tmp1, n );
      vec_pt_mul_so3( cosEval, tmp1, tmp2, n ) ;
      vec_mul_so3( L2_cN_so3(m+i, m1, m2), prev, tmp3, n ) ;
      vec_add_so3( tmp0, tmp2, tmp4, n );
      vec_add_so3( tmp3, tmp4, tmp5, n ); /* tmp5 contains d_{m1,m2}^{m+i+1} */

      /* store the function values */
      memcpy( result + (i+1)*n, tmp5, sizeof(double) * n );

      /* update for the next iteration of the recurrence */
      memcpy( prevprev, prev, sizeof( double ) * n ) ;
      memcpy( prev, tmp5, sizeof( double ) * n ) ;
    }
