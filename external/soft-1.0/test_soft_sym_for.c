...
/*
  ...
  input: - bandwidth bw 
         - input file names of real and imaginary parts of sample values ...
	   should be (2*bw)^3 values in each file
  ...
  example: test_soft_for_sym bw InputReal InputImag OutputReal OutputImag format

  example: test_soft_for_sym 16 rSam.dat iSam.dat rCoeffs.dat iCoeffs.dat 1
...

{
  ...
  if (argc < 2)
    {
      fprintf(stdout,"Usage: test_soft_for_sym bw realSam_file imSam_file ");
      fprintf(stdout,"realCoef_file imagCoef_file ");
  ...
  /* first the real part of the samples */
  fp = fopen( argv[2] , "r" );
  for ( i = 0 ; i < n3 ; i ++ )
    fscanf(fp, "%lf", rsignal+i) ;
  fclose ( fp ) ;

  /* now the imaginary part of the samples */
  fp = fopen( argv[3] , "r" );
  for ( i = 0 ; i < n3 ; i ++ )
