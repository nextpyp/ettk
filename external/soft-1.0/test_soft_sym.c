...
/*
  ...
  example: test_soft_sym bw howManyLoops rError.dat iError.dat
    

  example: test_soft_sym 16 10 rError.dat iError.dat
...

{
  ...
  if( argc > 3 )
	...
	fprintf( fp, "%d\t %.15f\n", k, rcoeffsIn[ k ] - rcoeffsOut[ k ]);
      fclose( fp ) ;
      
      fp = fopen( argv[ 4 ] , "w" );
      for( k = 0 ; k < totalCoeffs_so3( bw ) ; k++ )
	fprintf( fp, "%d\t %.15f\n", k, icoeffsIn[ k ] - icoeffsOut[ k ]);
      fclose( fp ) ;      
    }
