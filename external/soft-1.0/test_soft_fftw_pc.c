...
/*
  ...
  example: test_soft_fftw_pc bw loop realErrors imagErrors

  example: test_soft_fftw_pc 16 10 rError.dat iError.dat
...

{
  ...
  p3 = fftw_plan_many_dft( rank, na, howmany,
  ...
  if( argc > 3 )
	...
	fprintf( fp, "%d\t %.15f\n",
		 k, coeffsIn[ k ][0] - coeffsOut[ k ][0]);
      fclose( fp ) ;
      
      fp = fopen( argv[ 4 ] , "w" );
      for( k = 0 ; k < totalCoeffs_so3( bw ) ; k++ )
	fprintf( fp, "%d\t %.15f\n",
		 k, coeffsIn[ k ][1] - coeffsOut[ k ][1]);
      fclose( fp ) ;      
    }
